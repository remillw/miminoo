    ...
    <div class="stripe-server-upload">
        ...
        >
            ...
            <div v-if="!isUploading" class="upload-prompt">
                ...
                <p class="upload-info">Formats acceptés: JPG, PNG, PDF • Max 10MB par fichier</p>
        ...
        <div v-if="selectedFiles.length > 0 && !isUploading" class="selected-files">
            ...
            <div class="upload-actions">
                <button @click="startUpload" :disabled="selectedFiles.length === 0 || validationErrors.length > 0" class="upload-btn primary">
                    🚀 Uploader et lier au compte Stripe ({{ selectedFiles.length }} fichier{{ selectedFiles.length > 1 ? 's' : '' }})
                </button>
                <button @click="clearFiles" class="upload-btn secondary">Effacer</button>
            </div>
        </div>
        ...
        <div v-if="uploadedFiles.length > 0" class="upload-results">
            ...
            <div class="success-message">
                <p>
                    🎉 Vos documents ont été uploadés avec la clé secrète et automatiquement liés à votre compte Connect pour résoudre les
                    requirements!
                </p>
            </div>
            <div class="file-list">
...
// Composable
const { isUploading, uploadProgress, uploadedFiles, errors, uploadFiles, validateFiles, reset } = useStripeServerUpload();
...
const startUpload = async () => {
    ...
    if (validationErrors.value.length > 0) {
        ...
        emit(
            'upload-error',
            new Error('Erreurs de validation: ' + validationErrors.value.map((e) => `${e.file}: ${e.errors.join(', ')}`).join('; ')),
        );
        return;
    }

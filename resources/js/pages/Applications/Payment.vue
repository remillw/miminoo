            ...
            <div class="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">Réservation pour {{ application.ad.title }}</p>
                    </div>
                ...
                <div class="lg:col-span-1">
                    <div class="rounded-lg bg-white p-6 shadow-sm">
                        ...
                        <div class="mb-4 flex items-center gap-3">
                            <img
                                :src="application.babysitter.avatar || '/images/default-avatar.png'"
                        ...
                        <div class="mb-4 rounded-lg bg-gray-50 p-4">
                            <h3 class="font-medium text-gray-900">{{ application.ad.title }}</h3>
                            <p class="text-sm text-gray-600">
                                {{ formatDate(application.ad.date_start) }} - {{ formatDate(application.ad.date_end) }}
                            </p>
                        </div>
                        ...
                        <div class="space-y-2 border-t border-gray-200 pt-4">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-medium">{{ application.effective_rate }}€/h</span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Acompte (1h) :</span>
                                <span class="font-medium">{{ application.deposit_amount }}€</span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Frais de service :</span>
                                <span class="font-medium">{{ application.service_fee }}€</span>
                            </div>
                            <div class="flex justify-between border-t border-gray-200 pt-2 font-semibold">
                                <span>Total à payer :</span>
                                <span class="text-primary text-lg">{{ application.total_deposit }}€</span>
                            </div>
                            ...
                            <div class="mt-4 rounded-lg bg-gray-50 p-3">
                                ...
                                <div class="space-y-1 text-xs">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-medium text-gray-500">{{ application.stripe_fee }}€</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-gray-600">Montant reçu par la babysitter :</span>
                                        <span class="font-medium text-green-600">{{ application.babysitter_amount }}€</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-gray-600">Frais plateforme :</span>
                                        <span class="font-medium text-blue-600">{{ application.platform_fee }}€</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="rounded-lg bg-white p-6 shadow-sm">
                        ...
                        <div class="flex gap-3">
                            ...
                            >
                                ...
                                <span v-else>Confirmer le paiement ({{ application.total_deposit }}€)</span>
                            </button>
                        </div>
                    </div>
                </div>
...
onMounted(async () => {
    try {
        ...
        if (props.stripePublishableKey && StripeConstructor) {
            ...
            if (useNewPaymentMethod.value && props.application.client_secret) {
                await nextTick();
                try {
                    await initializeStripeElements(props.application.client_secret);
                } catch (error) {
...
watch(useNewPaymentMethod, async (newValue) => {
    if (newValue && stripe.value && !stripeElementsReady.value && props.application.client_secret) {
        try {
            await nextTick();
            await initializeStripeElements(props.application.client_secret);
        } catch (error) {
...
const confirmPayment = async () => {
    ...
    try {
        if (selectedPaymentMethod.value) {
            // Paiement avec moyen sauvegardé - TODO: implémenter si nécessaire
            paymentError.value = 'Paiement avec moyens sauvegardés non encore supporté pour les nouvelles réservations';
            ...
            // Confirmer côté serveur et créer la réservation
            router.post(
                route('applications.confirm-payment', props.application.id),
                {
...
const handlePaymentMethodChange = async () => {
    if (useNewPaymentMethod.value && stripe.value && props.application.client_secret) {
        await nextTick();
        try {
            await initializeStripeElements(props.application.client_secret);
        } catch (error) {

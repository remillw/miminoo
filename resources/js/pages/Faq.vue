  ...
  <GlobalLayout>
    <section class="min-h-[70vh] bg-white py-16 px-4">
        ...
        <div class="flex flex-wrap justify-center gap-3 mb-8">
          <button
            ...
            :class="[
              ...
              selectedCategory === cat.key ? 'bg-primary text-white border-primary shadow' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'
            ]"
          >
            {{ cat.name }}
          </button>
      ...
      <div class="max-w-2xl mx-auto">
        <div v-if="filteredFaqs.length === 0" class="text-center text-gray-400 py-12">
          <span>Aucune question trouvée pour cette catégorie.</span>
        </div>
        <div v-for="(faq, idx) in filteredFaqs" :key="faq.question" class="mb-4">
          <div
            class="flex items-center justify-between cursor-pointer rounded-xl border border-gray-200 bg-white px-6 py-5 shadow-sm hover:shadow-md transition"
            @click="toggleFaq(idx)"
          >
            <span class="font-semibold text-lg text-gray-900 text-left">{{ faq.question }}</span>
            <svg :class="['w-6 h-6 transition-transform', openIndex === idx ? 'rotate-180 text-primary' : 'text-gray-400']" fill="none" stroke="currentColor" viewBox="0 0 24 24">

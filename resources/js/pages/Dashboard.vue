...
onMounted(async () => {
    ...
    if (isNative.value && props.user) {
        console.log("🔔 Dashboard - Initialisation des notifications push pour l'utilisateur connecté");
        console.log('👤 Dashboard - User ID:', props.user.id);
        console.log('📱 Dashboard - Plateforme native confirmée:', isNative.value);

        try {
            await initializePushNotifications();
            console.log('✅ Dashboard - Push notifications initialisées avec succès');
        } catch (error) {
            console.error('❌ Dashboard - Erreur initialisation push notifications:', error);
        }
    } else {
        console.log('⚠️ Dashboard - Skip push notifications:', {
            isNative: isNative.value,
            hasUser: !!props.user,
        });
    }

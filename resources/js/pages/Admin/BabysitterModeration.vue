...
const handleVerify = async (babysitter: User, status: 'verified' | 'rejected') => {
    ...
    try {
        await router.post(`/admin/moderation-babysitters/${babysitter.id}/verify`, {
            status,
            ...
            onError: () => {
                showError(
                    'Erreur',
                    'Une erreur est survenue lors de la vérification du profil.'
                );
            }
        });
...
<template>
    ...
    <AdminLayout v-if="isAdmin" title="Modération des Babysitters" :description="`${pendingBabysitters.length} profil${pendingBabysitters.length !== 1 ? 's' : ''} en attente de vérification`">
        <!-- Header Section -->
        ...
        <!-- Statistiques -->
        <div v-if="pendingBabysitters.length === 0" class="text-center py-12">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div class="mt-2 space-y-2">
                                            ...
                                            <p class="text-sm"><span class="font-medium">Adresse:</span> {{ item.address?.address || 'Non renseignée' }}</p>
                                            <p class="text-sm"><span class="font-medium">Code postal:</span> {{ item.address?.postal_code || 'Non renseigné' }}</p>
                                        </div>
                                    </div>
                                    <div>
                                        <h4 class="font-medium text-gray-900">Profil babysitter</h4>
                                        <div class="mt-2 space-y-2">
                                            <p class="text-sm"><span class="font-medium">Expérience:</span> {{ item.babysitter_profile.experience_years || 0 }} année(s)</p>
                                            <p class="text-sm"><span class="font-medium">Tarif:</span> {{ item.babysitter_profile.hourly_rate || 'Non renseigné' }}€/h</p>
                                            <p class="text-sm"><span class="font-medium">Demande créée:</span> {{ formatDate(item.babysitter_profile.created_at) }}</p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <DialogContent>
                            ...
                            <div class="space-y-4">
                                <p class="text-sm text-gray-600">
                                    Vous êtes sur le point de rejeter le profil de {{ selectedBabysitter?.firstname }} {{ selectedBabysitter?.lastname }}.
                                ...
                                <div class="flex justify-end space-x-2">
                                    ...
                                    <Button
                                        variant="destructive"
                                        :loading="isProcessing"
                                        @click="handleVerify(selectedBabysitter!, 'rejected')"
                                    >
                                        Confirmer le rejet
                                    </Button>
                                </div>
